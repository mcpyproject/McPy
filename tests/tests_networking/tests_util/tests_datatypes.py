...
from networking.util.datatypes import VarInt, VarLong
...
def test_encode_varint():
    ...
    assert raises(struct.error, VarInt.encode, 2147483648)
    assert raises(struct.error, VarInt.encode, -2147483649)
    assert raises(struct.error, VarInt.encode, 23489062762390)
    assert raises(struct.error, VarInt.encode, -12045763498527)
    assert raises(struct.error, VarInt.encode, 324576230948720987534456)
    assert raises(struct.error, VarInt.encode, -293786529387462938547564)
...
def test_decode_varint_bytes():
    ...
    # Test invalid inputs
    assert raises(OverflowError, VarInt.decode, b"\xff\xff\xff\xff\xff\xff")
    assert raises(OverflowError, VarInt.decode, b"\x80\x80\x80\x80\x80\x80")
    assert raises(IndexError, VarInt.decode, b"\xff")
    assert raises(IndexError, VarInt.decode, b"")
...
def test_decode_varint_socket():
    ...
    assert raises(OverflowError, VarInt.decode, serv_c)
    assert serv_c.recv(4096) == b"\xca'\xdf"
    client.close()
    assert raises(TypeError, VarInt.decode, serv_c)
...
def test_encode_varlong():
    ...
    assert raises(struct.error, VarLong.encode, 349876025734624589762384762352)
    assert raises(struct.error, VarLong.encode, -104875623985426394857623954873)
    

def test_decode_varlong_bytes():
    assert VarLong.decode(b"\x00") == (0, b"")
    assert VarLong.decode(b"\x01") == (1, b"")
    assert VarLong.decode(b"\x02") == (2, b"")
    assert VarLong.decode(b"\x7f") == (127, b"")
    assert VarLong.decode(b"\x80\x01") == (128, b"")
    assert VarLong.decode(b"\xff\x01") == (255, b"")
    assert VarLong.decode(b"\xff\xff\xff\xff\x07") == (2147483647, b"")
    assert VarLong.decode(b"\xff\xff\xff\xff\xff\xff\xff\xff\x7f") == (9223372036854775807, b"")
    assert VarLong.decode(b"\xff\xff\xff\xff\xff\xff\xff\xff\xff\x01") == (-1, b"")
    assert VarLong.decode(b"\x80\x80\x80\x80\xf8\xff\xff\xff\xff\x01") == (-2147483648, b"")
    assert VarLong.decode(b"\x80\x80\x80\x80\x80\x80\x80\x80\x80\x01") == (-9223372036854775808, b"")
    
    # invalid inputs
    assert raises(OverflowError, VarLong.decode, b"\x80\x80\x80\x80\x80\x80\x80\x80\x80\x80")
    assert raises(OverflowError, VarLong.decode, b"\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff")
    assert raises(IndexError, VarLong.decode, b"\xff")
    assert raises(IndexError, VarLong.decode, b"")
...
def test_decode_varlong_socket():
    ...
    assert raises(OverflowError, VarLong.decode, serv_c)
    assert serv_c.recv(4096) == b"\xca'\xdf"
    client.close()
    assert raises(TypeError, VarLong.decode, serv_c)
